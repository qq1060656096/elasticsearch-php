...
*/
     ...
     * @return array|mixed
     */
    public function executeRequest($caller, $method, array $endpointParams, $expectedError, $expectedWarnings, $testName)
    {
        try {
    ...
    public function operationMatch($operation, $lastOperationResult, &$context, $testName)
    {
        ...
        if ($key === '$body') {
            ...
            if (empty($key)) {
                $match = isset($lastOperationResult['_source']) ? $lastOperationResult['_source'] :  $lastOperationResult;
            } else {
    ...
    public function operationGreaterThanOrEqual($operation, $lastOperationResult, &$context, $testName)
    {
        ...
        $expected = isset($context[current($operation)]) ? $context[current($operation)] :  current($operation);
